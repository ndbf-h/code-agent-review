 ...
 */
export function startSweeper(session: RabbitSession, producer: TaskProducer, config: AppConfig): Sweeper {
  ...
  async function sweep(): Promise<void> {
    ...
    for (const taskId of await findStaleRunningTasks(staleBefore)) {
      console.warn(`[sweeper] 任务 ${taskId} 心跳超时，重置回队列`)
      await appendTaskEvent(taskId, { type: 'task_retrying', message: '执行进程心跳丢失，任务已重新入队' })
        .catch(() => undefined)
      await producer.publishTask(taskId).catch(error => {
        console.error(`[sweeper] 任务 ${taskId} 重新入队失败:`, error instanceof Error ? error.message : error)
      })
    }
    ...
    for (const taskId of await findLostPendingTasks(lostBefore)) {
      console.warn(`[sweeper] 任务 ${taskId} 长时间 pending，补偿性重新发布`)
      await producer.publishTask(taskId).catch(error => {
        console.error(`[sweeper] 任务 ${taskId} 补偿发布失败:`, error instanceof Error ? error.message : error)
      })
    }
  }
  ...
  const timer = setInterval(() => {
    sweeping = sweeping.then(sweep).catch(error => {
      console.error('[sweeper] 扫描失败:', error instanceof Error ? error.message : error)
    })
  }, SWEEP_INTERVAL_MS)
  timer.unref?.()
