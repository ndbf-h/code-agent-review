...
function parseIpv6(ip: string): number[] | null {
  ...
  const groups = sections.length === 2
    ? [...left, ...Array(8 - groupCount).fill('0'), ...right]
    : left
...
export function isPrivateIp(ip: string): boolean {
  ...
  if (isIPv4(value)) {
    ...
    return inIpv4Range(number, '0.0.0.0', '0.255.255.255')
  ...
  if (isIPv6(value)) {
    ...
    if (isIpv4Compatible || (groups.slice(0, 5).every(group => group === 0) && groups[5] === 0xffff)) {
