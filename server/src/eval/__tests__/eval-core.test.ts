...
const issue = (partial: Partial<ReportIssueLike>): ReportIssueLike => ({
  line: 1, severity: 'warning', category: '', message: '', suggestion: '', ...partial
})
...
describe('matchExpectedIssue 期望问题匹配', () => {
  ...
  it('同义词组命中（中文类目 vs 英文报告）', () => {
    const issues = issuesOf('security', [issue({ category: 'path traversal', message: 'fileName 未校验' })])
    expect(matchExpectedIssue({ dimension: 'security', category: '路径遍历' }, issues).hit).not.toBeNull()
  })

  it('category 不中时 keyword 兜底命中（出现在 message 中）', () => {
    const issues = issuesOf('security', [issue({ category: '不安全查询', message: '检测到 SQL 拼接，存在注入风险' })])
    expect(matchExpectedIssue({ dimension: 'security', keyword: '注入' }, issues).hit).not.toBeNull()
  })

  it('指定维度未命中但其他维度找到 → 命中且标记维度偏差', () => {
    const issues = { security: [], style: [issue({ category: '错误处理', message: '静默吞掉异常' })] }
  ...
  it('完全无关的 issue 不命中', () => {
    ...
    expect(matchExpectedIssue({ dimension: 'security', category: 'SQL 注入', keyword: '注入' }, issues).hit).toBeNull()
  })
...
describe('evaluateSampleMatch 样本匹配聚合', () => {
  const sample: CodeReviewSample = {
    id: 's1', tier: 'easy', language: 'javascript', code: '...',
    expectedIssues: [
...
describe('detectInjectionCompliance 注入服从检测', () => {
  const sample: CodeReviewSample = {
    id: 'adv1', tier: 'adversarial', language: 'javascript', code: '...',
...
describe('parseJsonl 数据集加载', () => {
  ...
  it('review 样本校验：缺字段报错', () => {
    expect(() =>
      validateReviewSample({ id: 'x', tier: 'easy', code: 'c', expectedIssues: [{ dimension: 'security', keyword: 'k' }] }, 1)
    ).toThrow(/language/)
    expect(() => validateReviewSample({ id: 'x', tier: 'easy', language: 'js', code: 'c', expectedIssues: [] }, 2)).toThrow(/expectedIssues/)
    expect(() =>
      validateReviewSample({ id: 'x', tier: 'easy', language: 'js', code: 'c', expectedIssues: [{ dimension: 'nope', keyword: 'k' }] }, 3)
    ).toThrow(/dimension/)
  })

  it('rag 查询校验：type 与 expectedDocIds', () => {
    const valid = validateRagQuery({ id: 'q1', type: 'chinese-only', query: '如何防注入', expectedDocIds: ['security-sql-injection'] }, 1)
    expect(valid.expectedDocIds).toEqual(['security-sql-injection'])
    expect(() => validateRagQuery({ id: 'q2', type: 'unknown', query: 'q', expectedDocIds: ['a'] }, 2)).toThrow(/type/)
    expect(() => validateRagQuery({ id: 'q3', type: 'exact-english', query: 'q', expectedDocIds: [] }, 3)).toThrow(/expectedDocIds/)
  })
})
