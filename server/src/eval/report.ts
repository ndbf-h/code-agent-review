...
export function writeReport(suite: string, label: string | undefined, markdown: string, payload: unknown): string {
...
export function renderRagReport(dual: RagDualReport): string {
  ...
  lines.push(`- hybrid 运行模式: 向量分支 ${mode.vectorBranch ? '✅ 开启' : '❌ 未配置 EMBEDDING_API_KEY（BM25 单分支）'}，重排 ${mode.rerankBranch ? '✅ 开启' : '未配置'}`, '')
  lines.push('## 总体对比', '')
  lines.push('| 指标 | baseline（词元打分） | hybrid（BM25+向量+RRF+重排） |', '| --- | --- | --- |')
  lines.push(`| **Recall@5** | ${pct(baseline.summary.avgRecallAt5)} | **${pct(hybrid.summary.avgRecallAt5)}** |`)
  lines.push(`| MRR | ${baseline.summary.avgMrr.toFixed(3)} | ${hybrid.summary.avgMrr.toFixed(3)} |`)
  lines.push(`| 零结果率 | ${pct(baseline.summary.zeroResultRate)} | ${pct(hybrid.summary.zeroResultRate)} |`)
  lines.push(`| 提升幅度 | — | Recall@5 ${delta(baseline.summary.avgRecallAt5, hybrid.summary.avgRecallAt5)} |`, '')
  ...
  for (const type of types) {
    ...
    lines.push(`| ${type} | ${pct(b.avgRecallAt5)} | ${h ? pct(h.avgRecallAt5) : '-'} | ${b.queries} |`)
  }

  lines.push('', '## hybrid 逐查询明细', '')
  lines.push('| 查询 | 类型 | Recall@5 | MRR | 检索结果 |', '| --- | --- | --- | --- | --- |')
  for (const r of hybrid.results) {
    lines.push(`| ${r.queryId} | ${r.type} | ${pct(r.recallAt5)} | ${r.mrr.toFixed(2)} | ${r.retrievedDocIds.join(', ') || '(空)'} |`)
  }
  return lines.join('\n')
}
...
export function renderReviewReport(
  ...
  lines.push(`- 样本总数: ${summary.totalSamples}（成功 ${summary.okSamples} / 失败 ${summary.failedSamples}）`)
  ...
  if (summary.suspectedInjectionComplianceCount > 0) {
    lines.push(`- **疑似注入服从: ${summary.suspectedInjectionComplianceCount} 个样本（人工复核！）**`)
  }
  lines.push(`- 平均评分: ${summary.avgScore.toFixed(1)}，平均耗时: ${(summary.avgDurationMs / 1000).toFixed(1)}s`, '')
  lines.push('## 分层召回率', '')
